...
import { Camera, ExternalLink, MessageCircle, CheckCircle, AlertTriangle, Truck, Home, Sparkles, ChevronDown, ChevronUp, Loader2, Clock, Info, ShieldCheck, Lock } from 'lucide-react';
...
const statusColors: Record<ShipmentStatus, string> = {
  [ShipmentStatus.DELIVERED]: 'bg-emerald-100 text-emerald-800 border-emerald-200 ring-emerald-500 dark:bg-emerald-900/50 dark:text-emerald-300 dark:border-emerald-800',
  [ShipmentStatus.IN_TRANSIT]: 'bg-blue-100 text-blue-800 border-blue-200 ring-blue-500 dark:bg-blue-900/50 dark:text-blue-300 dark:border-blue-800',
  [ShipmentStatus.IN_OFFICE]: 'bg-amber-100 text-amber-800 border-amber-200 ring-amber-500 dark:bg-amber-900/50 dark:text-amber-300 dark:border-amber-800',
  [ShipmentStatus.ISSUE]: 'bg-rose-100 text-rose-800 border-rose-200 ring-rose-500 dark:bg-rose-900/50 dark:text-rose-300 dark:border-rose-800',
  [ShipmentStatus.PENDING]: 'bg-slate-100 text-slate-800 border-slate-200 ring-slate-400 dark:bg-slate-800 dark:text-slate-300 dark:border-slate-700',
};
...
export const ShipmentRow: React.FC<ShipmentRowProps> = ({ shipment, onUpdateStatus, onOpenEvidence, onToggleCheck, onUpdateSmartTracking }) => {
      ...
      >
        <td className="p-4">
          <div className="flex flex-col">
            <span className={`font-mono font-bold text-base tracking-wide flex items-center gap-2 ${shipment.checkStatus ? 'text-slate-500 dark:text-slate-500' : 'text-slate-800 dark:text-slate-200'}`}>
              {shipment.id}
        ...
        <td className="p-4">
          <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-bold uppercase tracking-wider shadow-sm border ${
             shipment.carrier === CarrierName.INTER_RAPIDISIMO ? 'bg-orange-50 text-orange-700 border-orange-100 dark:bg-orange-900/30 dark:text-orange-300 dark:border-orange-800' :
             shipment.carrier === CarrierName.ENVIA ? 'bg-red-50 text-red-700 border-red-100 dark:bg-red-900/30 dark:text-red-300 dark:border-red-800' :
             shipment.carrier === CarrierName.COORDINADORA ? 'bg-blue-50 text-blue-700 border-blue-100 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-800' :
             shipment.carrier === CarrierName.TCC ? 'bg-yellow-50 text-yellow-700 border-yellow-100 dark:bg-yellow-900/30 dark:text-yellow-300 dark:border-yellow-800' :
        ...
        <td className="p-4" onClick={(e) => e.stopPropagation()}>
          <div className="flex items-center gap-2">
            ...
            >
                ...
                {shipment.aiAnalysis && <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-purple-500 rounded-full border-2 border-white dark:border-slate-900 animate-pulse" />}
            </button>
            <button 
            ...
            >
                {isTracking ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5" />}
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-2 text-sm text-slate-600 dark:text-slate-300">
                            <p><strong className="text-slate-800 dark:text-slate-200">Transportadora:</strong> {shipment.carrier}</p>
                            <p><strong className="text-slate-800 dark:text-slate-200">Estado Local:</strong> {shipment.status}</p>
                            <p><strong className="text-slate-800 dark:text-slate-200">Teléfono:</strong> {shipment.phone || "No registrado"}</p>
                            <div className="pt-2">
                                <textarea 
                    ...
                    <div className="space-y-4">
                        ...
                        {isTracking ? (
                                ...
                                <div className="p-5 space-y-4">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            ...
                                            <p className="text-slate-800 dark:text-white font-bold text-lg leading-tight">{shipment.smartTracking.statusSummary}</p>
                                        </div>
                                        <div className="text-right">
                                             <p className="text-[10px] text-slate-400 font-bold uppercase mb-1">Tiempo</p>
                                             <p className="text-slate-600 dark:text-slate-300 font-mono text-sm">{shipment.smartTracking.timeElapsed}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className={`p-3 rounded-lg border-l-4 ${
                                        ...
                                        <p className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase mb-1">Recomendación Logística</p>
                            ...
                            <div className="h-40 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-dashed border-slate-300 dark:border-slate-700 flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 gap-2 p-6 text-center">
                                ...
                                <button onClick={handleSmartTrack} className="text-indigo-600 dark:text-indigo-400 font-bold text-sm hover:underline">
