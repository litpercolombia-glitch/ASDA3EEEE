...
import { Siren, AlertTriangle, Eye, CheckCircle, MapPin, Truck, Phone, Clock, MessageCircle, RefreshCw, Filter } from 'lucide-react';
...
export const AlertDashboard: React.FC<AlertDashboardProps> = ({ shipments, onSelectShipment }) => {
    const [filterCity, setFilterCity] = useState<'ALL' | 'BOGOTA' | 'MEDELLIN' | 'CALI' | 'OTROS'>('ALL');
    ...
    const filteredList = shipments.filter(s => {
        ...
        if (filterCity === 'OTROS' && (dest.includes('BOGOTA') || dest.includes('MEDELLIN') || dest.includes('CALI'))) return false;
    ...
    const AlertCard: React.FC<{shipment: Shipment}> = ({ shipment }) => {
        ...
        const colorClass = 
            risk?.level === ShipmentRiskLevel.URGENT ? 'border-red-500 bg-red-50 dark:bg-red-900/10' :
            risk?.level === ShipmentRiskLevel.ATTENTION ? 'border-amber-500 bg-amber-50 dark:bg-amber-900/10' :
            'border-yellow-400 bg-yellow-50 dark:bg-yellow-900/10';
        
        const badgeColor = 
            risk?.level === ShipmentRiskLevel.URGENT ? 'bg-red-500 text-white' :
            risk?.level === ShipmentRiskLevel.ATTENTION ? 'bg-amber-500 text-white' :
            ...
            >
                ...
                <div className="flex gap-4 mb-4 text-sm">
                    <div className="flex-1">
                        ...
                        <p className="font-medium text-slate-800 dark:text-slate-200 leading-tight">{risk?.reason}</p>
                    </div>
                    <div className="flex-1">
        ...
        <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-navy-900 text-white p-6 rounded-2xl shadow-xl flex flex-col md:flex-row justify-between items-center gap-4 relative overflow-hidden">
                ...
                <div className="flex gap-4">
                     <div className="text-center cursor-pointer hover:opacity-80 transition-opacity" onClick={() => setFilterRisk('URGENTE')}>
                     ...
                     <div className="text-center cursor-pointer hover:opacity-80 transition-opacity" onClick={() => setFilterRisk('ATTENTION')}>
                     ...
                     <div className="w-px bg-navy-700 h-10"></div>
                     <div className="text-center cursor-pointer hover:opacity-80 transition-opacity" onClick={() => setFilterRisk('NORMAL')}>
            ...
            <div className="flex flex-wrap gap-4 items-center bg-white dark:bg-navy-900 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-navy-800">
                <span className="text-xs font-bold uppercase text-slate-500 mr-2 flex items-center gap-1"><Filter className="w-3 h-3"/> Filtros de Riesgo:</span>
