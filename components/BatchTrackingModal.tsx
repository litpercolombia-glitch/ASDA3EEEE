...
export const BatchTrackingModal: React.FC<BatchTrackingModalProps> = ({
  ...
  const [detectedUpdates, setDetectedUpdates] = useState<
    { id: string; status: ShipmentStatus; rawStatus: string }[]
  >([]);
        ...
        <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Actualiza múltiples guías usando Inteligencia Visual
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="space-y-6">
              <div className="relative pl-8 border-l-2 border-slate-200 dark:border-slate-700">
                ...
                <h3 className="text-lg font-bold text-slate-800 dark:text-white mb-2">
                  Paso 1: Abrir Rastreo
                </h3>
                <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
                  Generamos un enlace inteligente con las{' '}
                  <strong>{Math.min(shipments.length, 40)} guías</strong> visibles.
                </p>
                <a
                  href={trackingUrl}
                  target="_blank"
              ...
              <div className="relative pl-8 border-l-2 border-slate-200 dark:border-slate-700 pb-2">
                <div
                  className={`absolute -left-[9px] top-0 w-4 h-4 rounded-full ring-4 ring-white dark:ring-slate-900 ${step >= 2 ? 'bg-indigo-500' : 'bg-slate-300 dark:bg-slate-700'}`}
                ></div>
                <h3 className="text-lg font-bold text-slate-800 dark:text-white mb-2">
                  Paso 2: Capturar y Pegar
                </h3>
                <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
                  En la página de 17TRACK, toma una captura de pantalla (Win+Shift+S) donde se vean
                  las guías y sus estados. Luego presiona <strong>Ctrl + V</strong> aquí.
                </p>
                  ...
                  {image ? (
                    <div className="flex flex-col items-center">
                      <img
                        src={image}
                        alt="Pasted"
                        className="h-32 object-contain mb-3 rounded shadow-sm opacity-80"
                      />
              ...
              <div className="flex-1 overflow-y-auto min-h-[200px] space-y-2 pr-2">
                {isProcessing ? (
                  ...
                  detectedUpdates.map((update, idx) => (
                    <div
                      key={idx}
                      className="bg-white dark:bg-slate-900 p-3 rounded-lg border border-slate-200 dark:border-slate-800 flex justify-between items-center shadow-sm"
                    >
                      <div>
                        ...
                        <span
                          className={`text-xs font-bold px-2 py-1 rounded border ${
                            update.status === ShipmentStatus.DELIVERED
                              ...
                              : update.status === ShipmentStatus.ISSUE
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-slate-400 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-xl">
                    <p className="text-sm text-center px-6">
                      Los resultados aparecerán aquí después de pegar la imagen.
                    </p>
                  </div>
                )}
              </div>
