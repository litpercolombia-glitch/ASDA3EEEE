...
export const QuickReferencePanel: React.FC<QuickReferencePanelProps> = ({ isOpen, onClose, shipments }) => {
            ...
            <div className="flex-1 overflow-y-auto p-2 bg-slate-50 dark:bg-navy-950">
                <div className="space-y-2">
                    {shipments.map((s, idx) => {
                        ...
                        return (
                            <div key={s.id} className="bg-white dark:bg-navy-900 p-3 rounded-lg border border-slate-200 dark:border-navy-800 shadow-sm text-xs">
                                <div className="flex justify-between items-start mb-1">
                                    <span className="font-bold text-slate-800 dark:text-slate-200">#{idx + 1} {s.id}</span>
                                    <button 
                                        onClick={() => handleCopy(s.id, `id-${s.id}`)}
                                        className="text-slate-400 hover:text-blue-500"
                                    >
                                        {copiedId === `id-${s.id}` ? <Check className="w-3 h-3 text-green-500"/> : <Copy className="w-3 h-3"/>}
                                    </button>
                                </div>
                                <div className="flex justify-between items-start mb-1">
                                    ...
                                    {s.phone && (
                                        <button 
                                            ...
                                            className="text-slate-400 hover:text-blue-500"
                                        >
                                            {copiedId === `phone-${s.id}` ? <Check className="w-3 h-3 text-green-500"/> : <Copy className="w-3 h-3"/>}
                                        </button>
                                    )}
                                </div>
                                <div className={`mt-2 p-1.5 rounded bg-slate-50 dark:bg-navy-950 border ${s.status === ShipmentStatus.ISSUE ? 'border-red-200 text-red-700' : 'border-slate-100 text-slate-600'}`}>
