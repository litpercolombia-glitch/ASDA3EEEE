    ...
    <div className="space-y-6 p-4 sm:p-6 animate-fade-in">
      ...
      {misMetricas && (
        <>
          ...
          {/* Gráfica de Análisis por Horario */}
          {misMetricas.avanzadas?.analisisPorHorario?.guiasPorHora && misMetricas.avanzadas.analisisPorHorario.guiasPorHora.length > 0 && (
            <div className="bg-white dark:bg-navy-800 rounded-xl border border-slate-200 dark:border-navy-700 p-6">
              <h3 className="font-semibold text-slate-800 dark:text-white mb-4 flex items-center gap-2">
              ...
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={misMetricas.avanzadas.analisisPorHorario.guiasPorHora}>
                  ...
                  <Tooltip
                    ...
                    formatter={(value: number, name: string) => [
                      name === 'guias' ? `${value} guías` : `${value.toFixed(0)}%`,
                      name === 'guias' ? 'Guías' : 'Eficiencia'
                    ]}
                  />
                  <Bar dataKey="guias" name="guias" radius={[4, 4, 0, 0]}>
                    {misMetricas.avanzadas.analisisPorHorario.guiasPorHora.map((entry, index) => (
                      <Cell
                        key={index}
                        fill={entry.eficiencia >= 80 ? '#10b981' : entry.eficiencia >= 60 ? '#f59e0b' : '#ef4444'}
                      />
                    ))}
                  </Bar>
                </BarChart>
              ...
              <div className="flex items-center justify-center gap-4 mt-2 text-xs text-slate-500">
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-emerald-500"></span> Eficiente (&ge;80%)</span>
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-500"></span> Regular (&ge;60%)</span>
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500"></span> Bajo (&lt;60%)</span>
              </div>
              <p className="text-xs text-slate-400 dark:text-slate-500 mt-2 text-center">
                Mejor hora: {misMetricas.avanzadas.analisisPorHorario.mejorHora} | Peor hora: {misMetricas.avanzadas.analisisPorHorario.peorHora}
              </p>
            </div>
          )}
