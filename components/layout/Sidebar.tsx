...
const SUB_MENUS: Record<MainSection, SubMenuItem[]> = {
  ...
  'video-editor': [
    { id: 'basico', icon: Play, label: 'Basico' },
...
export function Sidebar({ onLogout, onOpenChat, onOpenHelp, userName, userEmail }: SidebarProps) {
  ...
  const mainMenuItems: MenuItem[] = [
    ...
    { id: 'video-editor', icon: Film, label: 'Editor Video', isNew: true, subItems: SUB_MENUS['video-editor'] },
  ...
  const getActiveSubItem = (section: MainSection): string | undefined => {
    switch (section) {
      ...
      case 'video-editor': return activeVideoEditorTab;
  ...
  const handleSubItemClick = (section: MainSection, subItemId: string) => {
    switch (section) {
      ...
      case 'video-editor': setVideoEditorTab(subItemId as VideoEditorTab); break;
    }
  };
