...
export function DashboardML() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'transportadoras' | 'alertas'>(
    'overview'
  );
    ...
    return (
      <div className="w-full max-w-7xl mx-auto p-6">
        <div className="bg-red-50 border border-red-200 rounded-xl p-8 text-center">
          ...
          <h3 className="text-lg font-semibold text-red-800 mb-2">Error cargando dashboard</h3>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
              ...
              <div className="text-center">
                <div className="text-3xl font-bold">
                  {formatCurrency(kpis_avanzados.costo_por_entrega)}
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm text-green-600 font-semibold">
                      +
                      {(
                        (tendencias.entregas[tendencias.entregas.length - 1] /
                          tendencias.entregas[0] -
                          1) *
                        100
                      ).toFixed(1)}
                      %
            ...
            <div className="bg-white rounded-xl shadow-sm border p-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <StatBox
                  ...
                  color="orange"
                />
                <StatBox label="Modelos Activos" value={modelos_activos.length} color="green" />
                <StatBox
                  label="Transportadoras"
                  value={rendimiento_transportadoras.length}
                  color="blue"
                />
                <StatBox label="Ciudades Activas" value={top_ciudades.length} color="purple" />
              </div>
                ...
                {rendimiento_transportadoras.map((t, idx) => (
                  <tr key={idx} className="border-b hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-3 px-4 text-center text-green-600">
                      {formatNumber(t.entregas_exitosas)}
                    </td>
                    <td className="py-3 px-4 text-center text-orange-600">
                      {formatNumber(t.retrasos)}
                    </td>
                    ...
                    <td className="py-3 px-4 text-center">
                      <span
                        className={`font-semibold ${t.score_confiabilidad >= 90 ? 'text-green-600' : t.score_confiabilidad >= 80 ? 'text-blue-600' : 'text-orange-600'}`}
                      >
                    ...
                    <td className="py-3 px-4 text-center">
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-medium border ${CALIFICACION_COLORS[t.calificacion]}`}
                      >
                        {t.calificacion}
                      </span>
                    </td>
      ...
      <div className="mt-3 grid grid-cols-3 gap-2 text-center">
        ...
        <div>
          <div className="text-lg font-bold text-blue-600">
            {(modelo.precision * 100).toFixed(1)}%
          </div>
          <div className="text-xs text-gray-500">Precision</div>
        </div>
        <div>
          <div className="text-lg font-bold text-purple-600">
            {formatNumber(modelo.predicciones_hoy)}
          </div>
          <div className="text-xs text-gray-500">Hoy</div>
        </div>
      </div>
      <div className="mt-2 text-xs text-gray-400 text-center">
        v{modelo.version} · Entrenado:{' '}
        {new Date(modelo.fecha_entrenamiento).toLocaleDateString('es-CO')}
      </div>
    </div>
  );
}
