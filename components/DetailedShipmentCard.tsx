...
import { getLogisticsTemplates, CHATEA_PRO_URL, getShipmentRecommendation, generateClaimPDF } from '../services/logisticsService';
import { trackShipmentWithAI } from '../services/geminiService';
import { Truck, CheckCircle, AlertTriangle, MapPin, Copy, Image as ImageIcon, MessageCircle, Clock, FileText, Smartphone, Package, History, Box, Siren, Ticket, Loader2, Lightbulb, Scale, ChevronDown, ChevronUp, Map, ExternalLink, Lock, Sparkles, Calendar, ArrowRight } from 'lucide-react';
...
export const DetailedShipmentCard: React.FC<DetailedShipmentCardProps> = ({ shipment, index, onUpdateSmartTracking }) => {
    ...
    const getAlerts = () => {
        if (shipment.status === ShipmentStatus.IN_OFFICE) {
            ...
            if (daysInOffice > 4) {
                return { type: 'CRITICAL', msg: `URGENTE: ${daysInOffice} DÍAS EN OFICINA. ¡GENERAR TICKET YA!` };
            } else {
    ...
    const handleCopyAll = async () => {
        ...
        setTimeout(async () => {
            try {
                ...
                console.error(error);
                alert("Tu navegador no soporta copiar imagen y texto simultáneamente. Se copiará solo la imagen.");
                const blob = await generateImage();
                if (blob) {
    ...
    const getStatusColor = (status: ShipmentStatus) => {
        switch(status) {
            case ShipmentStatus.DELIVERED: return 'text-emerald-600 bg-emerald-50 border-emerald-200';
            case ShipmentStatus.ISSUE: return 'text-red-600 bg-red-50 border-red-200';
            case ShipmentStatus.IN_TRANSIT: return 'text-blue-600 bg-blue-50 border-blue-200';
            case ShipmentStatus.IN_OFFICE: return 'text-amber-600 bg-amber-50 border-amber-200';
        ...
        <div className="p-0 bg-white dark:bg-navy-900 text-slate-800 dark:text-slate-200 font-sans">
            <div className="p-4 md:p-6 border-b border-slate-100 dark:border-navy-800 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div className="bg-slate-50 dark:bg-navy-950 border border-slate-200 dark:border-navy-800 rounded-full px-4 py-2 w-full md:w-80 flex items-center justify-between">
                    <span className="font-mono font-bold text-slate-800 dark:text-slate-200 tracking-wider text-lg">{shipment.id}</span>
                    <button className="bg-navy-900 dark:bg-white text-white dark:text-navy-900 text-xs font-bold px-3 py-1.5 rounded-full">
                        Rastrear
                    </button>
                </div>
                <div className="text-right">
                    <p className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest mb-1">GUÍA - SIN NOVEDAD</p>
                ...
                <div className="md:w-1/3 p-6 md:p-8 flex flex-col justify-between border-r border-slate-100 dark:border-navy-800 order-2 md:order-1">
                    <div className="space-y-8">
                        <div className="hidden md:block text-right">
                            <span className="text-xs font-bold text-slate-900 dark:text-white underline decoration-2 decoration-slate-900 underline-offset-4 cursor-pointer">Ver menos estados</span>
                        </div>
                        <div className="mt-8 md:mt-0">
                            <div className="mb-6">
                                <p className="text-[10px] uppercase font-bold text-slate-400 mb-1">ORIGEN</p>
                                <p className="text-lg font-bold text-orange-600 dark:text-orange-500 leading-tight">{info.origin.replace('COL', '')}</p>
                            </div>
                            ...
                            <div>
                                ...
                                <p className="text-lg font-bold text-orange-600 dark:text-orange-500 leading-tight">{info.destination.replace('COL', '')}</p>
                            </div>
                        </div>
                    </div>
                    <div className="mt-8">
                        ...
                        {eventsChronological.map((event, idx) => (
                            <div key={idx} className="relative pl-6">
                                ...
                                <div className="group">
                                    ...
                                    <p className="text-sm md:text-base font-bold text-slate-800 dark:text-slate-200 mb-1">{event.description}</p>
    ...
    const RenderEnviaHistory = () => (
        <div className="bg-white p-0 font-sans text-gray-800">
                    ...
                    <div className="space-y-8">
                        {eventsChronological.map((event, idx) => (
                            <div key={idx} className="relative flex gap-6">
                                <div className={`relative z-10 w-8 h-8 rounded-full flex items-center justify-center shrink-0 border-4 border-white shadow-sm ${idx === 0 ? 'bg-red-600 text-white' : 'bg-gray-200 text-gray-400'}`}>
                                    {idx === 0 ? <MapPin className="w-4 h-4" /> : <div className="w-2 h-2 bg-gray-400 rounded-full"></div>}
                                </div>
                                <div className={`flex-1 p-4 rounded-lg border ${idx === 0 ? 'bg-red-50 border-red-100' : 'bg-white border-gray-100 shadow-sm'}`}>
                                    <div className="flex justify-between items-start mb-2">
                                        <p className={`font-bold text-sm ${idx === 0 ? 'text-red-700' : 'text-gray-800'}`}>
                                            {event.description}
                                        </p>
            ...
            <div className="bg-gray-50 p-3 text-center border-t border-gray-200">
                <p className="text-[10px] text-gray-400 font-bold uppercase">Servicio de Mensajería y Mercancías</p>
            </div>
        </div>
    );
    ...
    const isRestricted = shipment.smartTracking?.statusSummary.toLowerCase().includes('validación') || 
        ...
        <div className="bg-white dark:bg-navy-900 rounded-lg shadow-sm border border-slate-200 dark:border-navy-700 overflow-hidden mb-1 transition-all hover:shadow-md animate-in fade-in duration-300">
            ...
            >
                <div className="flex items-center gap-3 flex-1 overflow-hidden">
                    <span className="text-[10px] text-slate-400 font-mono min-w-[20px] hidden sm:block">#{index + 1}</span>
                    
                    {/* Guide */}
                    <div 
                        onClick={(e) => { e.stopPropagation(); copyToClipboard(shipment.id); }}
                    ...
                    >
                        <span className="font-mono font-bold text-slate-800 dark:text-slate-200 text-sm">{shipment.id}</span>
                        <Copy className="w-3 h-3 text-slate-300 opacity-0 group-hover:opacity-100" />
                    </div>

                    {/* Phone - Now prominently next to guide */}
                    <div 
                        onClick={(e) => { e.stopPropagation(); copyToClipboard(shipment.phone); }}
                    ...
                    >
                        ...
                        <span className="font-mono text-sm text-slate-600 dark:text-slate-300">{shipment.phone || "---"}</span>
                        <Copy className="w-3 h-3 text-slate-300 opacity-0 group-hover:opacity-100" />
                    </div>

                    {/* Status Badge */}
                    <div 
                        onClick={(e) => { e.stopPropagation(); copyToClipboard(shipment.status); }}
            ...
            {isExpanded && (
                <div className="border-t border-slate-200 dark:border-navy-700 bg-slate-50 dark:bg-navy-950 p-4">
                    <div className="mb-4">
                        ...
                        <div ref={cardRef} className="bg-white dark:bg-navy-900 rounded-2xl shadow-xl border border-slate-200 dark:border-navy-700 overflow-hidden relative">
                            {activeTab === 'SUMMARY' && (
                                <div className="p-6 md:p-8">
                                    ...
                                    <div className="mb-6">
                                        {isTracking ? (
                                            <div className="h-20 bg-white dark:bg-slate-800 rounded-xl border border-purple-100 dark:border-purple-900 shadow-sm flex items-center justify-center text-purple-500 gap-3">
                                                ...
                                                <span className="text-sm font-medium">Conectando con {shipment.carrier}...</span>
                                            </div>
                                        ) : shipment.smartTracking ? (
                                            <div className={`bg-white dark:bg-slate-800 rounded-xl border shadow-sm overflow-hidden relative ${
                                                isRestricted ? 'border-amber-200 dark:border-amber-800' : 'border-slate-200 dark:border-slate-700'
                                            }`}>
                                                <div className={`p-2 text-white flex justify-between items-center ${
                                                    isRestricted ? 'bg-gradient-to-r from-amber-500 to-orange-500' : 'bg-gradient-to-r from-purple-600 to-indigo-600'
                                                }`}>
                                                ...
                                                <div className="p-4 flex flex-col md:flex-row gap-4 items-center justify-between">
                                                    <div>
                                                        <p className="text-[10px] text-slate-400 font-bold uppercase mb-1">Estado Detectado</p>
                                                        <p className="text-slate-800 dark:text-white font-bold text-base leading-tight">{shipment.smartTracking.statusSummary}</p>
                                                    </div>
                                                    <div className="flex-1 border-l border-slate-100 dark:border-slate-700 pl-4">
                                                         <p className="text-[10px] text-slate-400 font-bold uppercase mb-1">Recomendación Logística</p>
                                                         <p className="text-xs text-slate-600 dark:text-slate-300 leading-snug">{shipment.smartTracking.recommendation}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        ) : (
                                    ...
                                    {/* ALERT SECTION */}
                                    {shipmentAlert && (
                                        <div className={`mb-6 p-4 rounded-xl border-l-4 shadow-sm flex items-start gap-4 hover:scale-[1.01] transition-transform duration-300 ${
                                            shipmentAlert.type === 'CRITICAL' ? 'bg-red-50 dark:bg-red-900/10 border-red-500 text-red-800' :
                                            shipmentAlert.type === 'WARNING' ? 'bg-amber-50 dark:bg-amber-900/10 border-amber-500 text-amber-800' :
                                            'bg-blue-50 dark:bg-blue-900/10 border-blue-500 text-blue-800'
                                        }`}>
                                            <div className={`p-2 rounded-full ${
                                                shipmentAlert.type === 'CRITICAL' ? 'bg-red-100 text-red-600' :
                                                shipmentAlert.type === 'WARNING' ? 'bg-amber-100 text-amber-600' : 'bg-blue-100 text-blue-600'
                                            }`}>
                                                <Siren className={`w-6 h-6 ${shipmentAlert.type === 'CRITICAL' ? 'animate-pulse' : ''}`} />
                                            </div>
                                            <div className="flex-1">
                                                <h4 className="font-black text-sm uppercase tracking-wider mb-1">
                                                    {shipmentAlert.type === 'CRITICAL' ? 'ALERTA CRÍTICA DE TIEMPO' : 
                                                    shipmentAlert.type === 'WARNING' ? 'ATENCIÓN REQUERIDA' : 'INFORMACIÓN LOGÍSTICA'}
                                                </h4>
                                    ...
                                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 border-b border-slate-100 dark:border-navy-800 pb-4 gap-4">
                                        <div>
                                            <div className="flex items-center gap-2 mb-1">
                                                <span className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Guía de Transporte</span>
                                                <span className="bg-slate-100 dark:bg-navy-800 text-slate-500 dark:text-slate-300 text-[10px] px-2 py-0.5 rounded-md font-bold">#{index + 1}</span>
                                            </div>
                                            <div className="flex flex-wrap items-center gap-3">
                                                <h2 className="text-3xl font-black text-slate-800 dark:text-white tracking-tight">{shipment.id}</h2>
                                        ...
                                        <div className={`px-4 py-2 rounded-full font-bold text-sm flex items-center gap-2 border shadow-sm ${getStatusColor(shipment.status)}`}>
                                            {shipment.status === ShipmentStatus.DELIVERED ? <CheckCircle className="w-4 h-4"/> : 
                                            shipment.status === ShipmentStatus.IN_TRANSIT ? <Truck className="w-4 h-4"/> : 
                                            shipment.status === ShipmentStatus.ISSUE ? <AlertTriangle className="w-4 h-4"/> : <Box className="w-4 h-4"/>}
                                            {shipment.status}
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                        ...
                                        <div className="bg-slate-50 dark:bg-navy-950 rounded-2xl p-6 border border-slate-100 dark:border-navy-800 relative group overflow-hidden hover:scale-[1.02] hover:shadow-lg transition-all duration-300">
                                            ...
                                            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Ruta Logística</h3>
                                            ...
                                            <div className="space-y-6 relative z-10">
                                                <div className="flex items-start gap-3">
                                                    ...
                                                    <div className="flex-1 space-y-4">
                                                        <div>
                                                            ...
                                                            <p className="text-lg font-bold text-slate-800 dark:text-slate-100 leading-none">{info.origin}</p>
                                                        </div>
                                                        <div>
                                                            <p className="text-xs text-slate-500 uppercase font-semibold">Destino</p>
                                                            <p className="text-xl font-black text-orange-600 dark:text-orange-500 leading-none">{info.destination}</p>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div className="pt-4 border-t border-slate-200 dark:border-navy-800 flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-sm font-bold text-slate-800 dark:text-white">{shipment.carrier}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="bg-slate-50 dark:bg-navy-950 rounded-2xl p-6 border border-slate-100 dark:border-navy-800 relative group overflow-hidden hover:scale-[1.02] hover:shadow-lg transition-all duration-300">
                                            ...
                                            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Tiempos & Estado</h3>
                                            
                                            <div className="space-y-4 relative z-10">
                                                <div className="flex items-end gap-2">
                                                    <span className="text-5xl font-black text-slate-800 dark:text-white tracking-tighter">{info.daysInTransit}</span>
                                                    <span className="text-sm font-bold text-slate-500 pb-2 uppercase">Días en proceso</span>
                                                </div>
                                                
                                                <div className={`p-4 rounded-xl border-l-4 shadow-sm bg-white dark:bg-navy-900 ${
                                                    shipment.status === ShipmentStatus.DELIVERED ? 'border-emerald-500' :
                                                    shipment.status === ShipmentStatus.ISSUE ? 'border-red-500' :
                                                    'border-blue-500'
                                                }`}>
                                                    <p className="text-[10px] font-bold uppercase opacity-70 mb-1 text-slate-500">Último Movimiento (Estado Actual)</p>
                                        ...
                                        <div className="bg-slate-50 dark:bg-navy-950 rounded-2xl p-6 border border-slate-100 dark:border-navy-800 relative group overflow-hidden flex flex-col justify-between hover:scale-[1.02] hover:shadow-lg transition-all duration-300">
                                            <div>
                                                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Gestión & Plantillas</h3>
                                                ...
                                                <div className="space-y-3">
                                                    ...
                                                    >
                                                        <span className="flex items-center gap-2"><Smartphone className="w-4 h-4"/> Chatea Pro</span>
                                            ...
                                            <div className="mt-4 pt-4 border-t border-slate-200 dark:border-navy-800 space-y-2">
                                                ...
                                                {(shipment.status === ShipmentStatus.ISSUE || shipment.status === ShipmentStatus.IN_OFFICE) && (
                                                    <button 
                            ...
                            {activeTab === 'HISTORY' && (
                                <>
                                    ...
                                    {(shipment.carrier !== CarrierName.ENVIA && shipment.carrier !== CarrierName.COORDINADORA) && <RenderInterHistory />}
                                </>
                            )}
                        </div>
                        ...
                        <div className="flex justify-end gap-3 mt-4">
                            ...
                            <button 
                                ...
                                className="bg-white dark:bg-navy-900 border border-slate-200 dark:border-navy-700 text-slate-600 dark:text-slate-300 hover:text-orange-600 hover:border-orange-200 px-4 py-2.5 rounded-xl font-bold text-xs shadow-sm flex items-center gap-2 transition-all"
                            >
                                {isImageCopying ? <Loader2 className="w-4 h-4 animate-spin"/> : <ImageIcon className="w-4 h-4" />} 
                                Copiar Imagen
                            </button>
                            <button 
                            ...
                            >
                                {isCopying ? <Loader2 className="w-4 h-4 animate-spin" /> : <MessageCircle className="w-4 h-4" />}
                                Contactar Cliente
                            </button>
                        </div>
                    </div>
