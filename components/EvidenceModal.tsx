    ...
    const svgContent = (
      ...
      >
        ...
        <rect
          width="100%"
          height="10"
          fill={shipment.status === ShipmentStatus.DELIVERED ? '#22c55e' : '#ef4444'}
        />

        <text x="40" y="60" fontSize="24" fontWeight="bold" fill="#333">
          REPORTE DE NOVEDAD
        </text>
        <text x="40" y="90" fontSize="16" fill="#666">
          Litper Logística
        </text>

        <text x="40" y="140" fontSize="14" fill="#888">
          GUÍA:
        </text>
        <text x="40" y="165" fontSize="20" fontWeight="bold" fill="#000">
          {shipment.id}
        </text>

        <text x="300" y="140" fontSize="14" fill="#888">
          ESTADO:
        </text>
        <text
          x="300"
        ...
        <text x="40" y="210" fontSize="14" fill="#888">
          FECHA:
        </text>
        <text x="40" y="235" fontSize="16" fill="#000">
        ...
        <text x="60" y="610" fontSize="14" fontWeight="bold" fill="#475569">
          Análisis IA:
        </text>
      ...
      >
        <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center sticky top-0 bg-white dark:bg-slate-900 z-10">
          <h2 className="text-xl font-bold text-gray-800 dark:text-white">
            Evidencia Fotográfica - {shipment.id}
          </h2>
          <button
            onClick={onClose}
            className="p-2 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-full text-slate-500 dark:text-slate-400"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 border-t border-slate-100 dark:border-slate-800 bg-gray-50 dark:bg-slate-800/50 flex justify-end gap-3 sticky bottom-0 rounded-b-2xl">
          <button
            onClick={onClose}
            className="px-4 py-2 text-gray-600 dark:text-slate-400 hover:text-gray-800 dark:hover:text-white font-medium"
          >
            Cancelar
          </button>
          <button
